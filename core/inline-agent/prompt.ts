...
const OFFICECLI_TOOL_PREFIX = 'officecli_';
const OFFICECLI_CREATE_DOCUMENT = 'officecli_create_document';
const OFFICECLI_APPLY_EDIT_PLAN = 'officecli_apply_edit_plan';
const OFFICECLI_VALIDATE = 'officecli_validate';
const OFFICECLI_EXPORT_PREVIEW = 'officecli_export_preview';
const OFFICECLI_INSPECT = 'officecli_inspect';
const OFFICE_DOCUMENT_CREATE_RE = /(?:创建|生成|制作|新建|做一个|做一份|create|generate|make|build).{0,80}(?:ppt|pptx|powerpoint|幻灯片|演示文稿|文档|docx|word|表格|xlsx|excel|presentation|slides?|document|spreadsheet)/i;
...
export function shouldNudge(
  ...
  if (isOfficeCliDocumentTaskIncomplete(originalTask, executions)) return true;
...
export function buildContinuationPrompt(originalTask: string, executions: ToolExecutionRecord[]): string {
  ...
  const officeCliIncomplete = isOfficeCliDocumentTaskIncomplete(originalTask, executions);
  ...
  return [
    ...
    ...(officeCliIncomplete ? [
      '当前 OfficeCLI 文档创建任务还没有完成。',
      '请继续调用 officecli_apply_edit_plan 写入实际内容，并调用 officecli_validate 或 officecli_export_preview/officecli_inspect 验证。',
    ] : []),
    '',
    '<original_task>',
...
function isOfficeCliDocumentTaskIncomplete(originalTask: string, executions: ToolExecutionRecord[]): boolean {
  if (!OFFICE_DOCUMENT_CREATE_RE.test(originalTask)) return false;
  const officeExecutions = executions.filter((e) => e.name.startsWith(OFFICECLI_TOOL_PREFIX));
  if (officeExecutions.length === 0) return false;

  const hasCreate = officeExecutions.some((e) => e.name === OFFICECLI_CREATE_DOCUMENT && e.result.ok);
  const hasApply = officeExecutions.some((e) => e.name === OFFICECLI_APPLY_EDIT_PLAN && e.result.ok);
  const hasValidate = officeExecutions.some((e) => e.name === OFFICECLI_VALIDATE && e.result.ok);
  const hasReadback = officeExecutions.some((e) =>
    (e.name === OFFICECLI_INSPECT || e.name === OFFICECLI_EXPORT_PREVIEW) && e.result.ok,
  );

  if (!hasCreate) return true;
  if (!hasApply) return true;
  return !hasValidate && !hasReadback;
}
